...
Speech detection (Auto-detect, and Trim silence on a file with no fragments) runs Silero VAD in a Web Worker (`src/utils/vadWorker.ts`, started by `detectSpeechSegments()`), falling back to the main thread only if the worker cannot start. On the main thread it froze the page for minutes on a phone. `detectSpeechSegments()` transfers its sample buffer to the worker, so callers must not reuse it. MP3s are decoded at their own sample rate for processing (`streamAudioChunks`), not the device's 48 kHz, because that resample was nearly half the decode time. `wrapHeavyOp` holds a screen wake lock so the phone doesn't sleep mid-operation.

Progress of these operations goes through an `OperationProgress` tracker (`src/utils/operationProgress.ts`): the page lists the stages up front (e.g. decode → vad → trim → save), passes `tracker.setProgress` as each stage's progress callback and calls `startStage()` between them and `finish()` on success. The tracker turns them into one 0–100% and a time-left estimate. Its per-stage speeds start from per-device guesses and are learned from finished runs (`lingodrill.opRates` in localStorage). `OperationProgressIndicator` polls it every 500 ms — an inline bar in the action bar plus a pop-up with the up-front estimate that opens for runs expected to take ≥ 8 s (or still going after 3 s) and can be hidden. A new long operation should use the same tracker rather than raw percent state, so the page doesn't re-render on every callback.
